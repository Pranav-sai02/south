...
export class AreaCodesComponent implements OnInit {
    ...
    if (isNew) {
      this.areaCodesService.addAreaCode(row).subscribe({
        ...
        error: (err) => {
          ...
          this.toasterService.showError('Error Saving Area Code ');
          console.error(err);
        },
      });
    } else {
      ...
      this.areaCodesService
        ...
        .subscribe({
          ...
          error: (err) => {
            this.toasterService.showError('Error updating area code.');
            console.error(err);
          },
        });
    }
  ...
  softDeleteProvider(areaCode: AreaCodes): void {
    ...
    this.store.dispatch(new SoftDeleteAreaCode(updatedAreaCode)).subscribe({
      ...
      error: (err) => {
        this.toasterService.showError('Error Removing  area code.');
        console.error(err);
      },
    });
  }
