...
export class ClientGroupService {
  ...
  getClientGroups(): Observable<ClientGroup[]> {
    return this.http.get<ClientGroup[]>(this.apiUrl).pipe(
      catchError(error => this.handleError(error, 'getClientGroups'))
    );
  }

  /** POST add new client group */
  addClientGroup(clientGroup: ClientGroup): Observable<ClientGroup> {
    return this.http.post<ClientGroup>(this.apiUrl, clientGroup).pipe(
      catchError(error => this.handleError(error, 'addClientGroup'))
    );
  }

  /** PUT update existing client group */
  updateClientGroup(clientGroup: ClientGroup): Observable<ClientGroup> {
    return this.http.put<ClientGroup>(`${this.apiUrl}/${clientGroup.ClientGroupId}`, clientGroup).pipe(
  ...
  deleteClientGroup(id: number): Observable<ClientGroup> {
    return this.http.delete<ClientGroup>(`${this.apiUrl}/${id}`).pipe(
  ...
  softDeleteClientGroup(id: number): Observable<ClientGroup> {
    return this.http.patch<ClientGroup>(`${this.apiUrl}/${id}`, { IsDeleted: true }).pipe(
