...
export class ClientGroupState {
  ...
  updateClientGroup(
    ...
    const updatedClientGroup = state.clientGroups.map((clientGroup) =>
      clientGroup.ClientGroupId === action.payload.ClientGroupId ? action.payload : clientGroup
    );
    ctx.patchState({
  ...
  removeClientGroup(ctx: StateContext<ClientGroupStateModel>, action: RemoveClientGroup) {
